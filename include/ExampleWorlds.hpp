...
namespace ExampleWorlds
{
    ...
    std::shared_ptr<World> GetGetSquareWorld(size_t width, size_t height, size_t numRobots, double robotSize, size_t numPucks, double puckSize)
    {
        ...
        for (size_t r = 0; r < numRobots; r++)
        {
            ...
            sensors.gridSensors.push_back(std::make_shared<GridSensor>(robot, "gridRight", 0, 45, robotSize * 2));
            sensors.gridSensors.push_back(std::make_shared<GridSensor>(robot, "gridCentre", 0, 0, robotSize * 2));
            sensors.gridSensors.push_back(std::make_shared<GridSensor>(robot, "gridLeft", 0, -45, robotSize * 2));
            sensors.puckSensors.push_back(std::make_shared<PuckSensor>(robot, "leftPucks", "puck", -30, robotSize * 4, robotSize * 2));
            sensors.puckSensors.push_back(std::make_shared<PuckSensor>(robot, "rightPucks", "puck", 30, robotSize * 4, robotSize * 2));
            sensors.puckSensors.push_back(std::make_shared<PuckSensor>(robot, "farRightPucks", "puck", 60, robotSize * 7, robotSize * 2));
            sensors.puckSensors.push_back(std::make_shared<PuckSensor>(robot, "farLeftPucks", "puck", -60, robotSize * 7, robotSize * 2));
            sensors.obstacleSensors.push_back(std::make_shared<ObstacleSensor>(robot, "rightObs", 45, robotSize, robotSize/4));
            sensors.obstacleSensors.push_back(std::make_shared<ObstacleSensor>(robot, "leftObs", -45, robotSize, robotSize/4));
            sensors.robotSensors.push_back(std::make_shared<RobotSensor>(robot, "rightRobot", 30, robotSize * 2, robotSize));
            sensors.robotSensors.push_back(std::make_shared<RobotSensor>(robot, "leftRobot", -30, robotSize * 2, robotSize));
        }
